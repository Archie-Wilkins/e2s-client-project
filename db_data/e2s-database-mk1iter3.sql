...
CREATE TABLE IF NOT EXISTS `floors` (
  ...
  CONSTRAINT `FK__site_details` FOREIGN KEY (`site_id`) REFERENCES `site_details` (`site_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE IF NOT EXISTS `site_details` (
  ...
  `site_org_id` int(11) DEFAULT NULL,
  PRIMARY KEY (`site_id`),
  KEY `FK_site_details_organisations` (`site_org_id`),
  CONSTRAINT `FK_site_details_organisations` FOREIGN KEY (`site_org_id`) REFERENCES `organisations` (`org_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_520_ci;

-- Dumping data for table e2s_db.site_details: ~3 rows (approximately)
/*!40000 ALTER TABLE `site_details` DISABLE KEYS */;
INSERT INTO `site_details` (`site_id`, `site_name`, `post_code`, `address_l1`, `address_l2`, `county`, `site_size_x`, `site_size_y`, `site_org_id`) VALUES
	(1, NULL, 'ST21 C45', '21 Fictional Way', 'Building 5', 'Worcestershire', 20, 50, NULL),
	(2, NULL, 'EE4 A83', '15 Made This Up Place', '0', 'Devon', 234, 43, NULL),
	(3, NULL, 'AA93 Y56', '99 Road Street', 'ADR2', 'Scotland', 80, 80, NULL);
...
INSERT INTO `site_energy_data` (`entry_id`, `site_id`, `supplier_id`, `energy_hour_usage`, `energy_hour_cost`, `energy_hour_output`, `energy_hour_imported`, `energy_hour_exported`, `temp_average`, `wind_speed`, `carbon_day_emitted`, `time_stamp`) VALUES
